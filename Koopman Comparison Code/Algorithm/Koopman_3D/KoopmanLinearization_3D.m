function [K,init1,init2,init3] = KoopmanLinearization_3D(dynamics, p, T, N, x0, r,option)
...
% compute Gauss-Lobatto points and differentiation matrices
[x_GL, D_r] = compute_diffMat_3D(N, lb, ub, op);

% compute finite dimensional Koopman approximation matrix K
K = approximate_Koopman_3D(N,x_GL, D_r, dynamics, p);
% compute projection coefficients g_0
% Now default g(x)=[x1;x2;x3]
[init1,init2,init3]=compute_initial_3D(x_GL);
